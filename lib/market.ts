...
import { TokenBalance, MarketPrice, PositionWithPnL, CurrentPosition } from "./types"
import { MARKET_SYMBOL_MAP, MARKET_API_URL, MARKET_SYMBOLS, TOKEN_LIST } from "./utils"

export function calculatePortfolioValue(portfolio: TokenBalance[], marketPrices: MarketPrice[]): { positions: { symbol: string; balance: string; price: number; usd_value: number }[], totalUsd: number } {
    const positions = portfolio.map(token => {
      const price = token.symbol === 'USDC' ? 1 : getTokenPrice(token.symbol, marketPrices)
  ...
  export function calculatePositionsPnL(currentPositions: CurrentPosition[], marketPrices: MarketPrice[], portfolio: TokenBalance[]): { positionsWithPnl: PositionWithPnL[], totalUsd: number, totalPnl: number } {
    const positionsWithPnl = currentPositions.map(position => {
      ...
      const pnlPercent = position.totalInvested > 0 ? (pnlUsd / position.totalInvested) * 100 : 0
    ...
    const totalUsd = positionsWithPnl.reduce((sum, pos) => sum + pos.currentValue, 0)
  ...
  function getTokenPrice(symbol: string, marketPrices: MarketPrice[]): number {
    const marketSymbol = MARKET_SYMBOL_MAP[symbol as keyof typeof MARKET_SYMBOL_MAP]
  ...
  export async function fetchMarketPrices(): Promise<{ marketPrices: MarketPrice[], marketOverviewData: string }> {
    try {
    ...
    return { marketPrices: [], marketOverviewData: 'Market overview unavailable' }
  }
  

  export async function fetchPortfolioBalances(account: Account): Promise<TokenBalance[]> {
    const balancePromises = TOKEN_LIST.map(async (token) => {
      try {
        ...
        return {
          ...
          balanceFormatted: (Number(balance) / Math.pow(10, token.decimals)).toFixed(6)
        }
      } catch (error) {
