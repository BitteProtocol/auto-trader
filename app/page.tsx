		...
		const fetchData = async () => {
			try {
				const response = await fetch(
					`/api/dashboard?accountId=${NEAR_ACCOUNT_ID}`,
				);
			...
			<div className="flex flex-col lg:flex-row gap-4 lg:gap-8 pb-2">
				<div className="flex flex-col space-y-3 lg:min-w-[500px]">
					...
					<div className="text-2xl md:text-3xl font-semibold text-muted-foreground">
						...
						{(data.goalValue || 0).toLocaleString()}
					</div>
				</div>
				...
				<div className="flex flex-col sm:flex-row gap-4 lg:gap-8 flex-1">
					<div className="space-y-3 pt-2">
						<div
							className={`text-lg md:text-xl font-semibold flex items-center gap-2 ${(data.accruedYield || 0) >= 0 ? "text-green-500" : "text-red-500"}`}
						>
							<svg
								className="w-5 h-5"
								fill="none"
							...
							Accrued Yield: {(data.accruedYield || 0) >= 0 ? "+" : ""}$
							{(data.accruedYield || 0).toLocaleString()} (
							{(data.accruedYield || 0) >= 0 ? "+" : ""}
							{(data.yieldPercent || 0).toFixed(1)}%)
						</div>
						<div className="w-full max-w-lg">
							<div className="flex items-center justify-between text-sm mb-2">
								...
								<span className="font-semibold text-green-500">
									{data.goalValue > 0
										? (((data.totalValue || 0) / data.goalValue) * 100).toFixed(
												1,
											)
										: "0.0"}
									%
							...
							<div className="h-3 bg-muted/30 rounded-full overflow-hidden relative">
								<div
									...
									style={{
										width: `${Math.min(data.goalValue > 0 ? ((data.totalValue || 0) / data.goalValue) * 100 : 0, 100)}%`,
									}}
								>
							...
							<div className="flex items-center justify-between text-xs text-muted-foreground mt-2">
								<span>
									Started: ${(data.startingValue || 0).toLocaleString()}
								</span>
